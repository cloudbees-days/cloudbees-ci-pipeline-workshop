...
10. Next, open the GitHub editor for the **nodejs-pod.yaml** Pipeline script in the **master** branch of your forked **helloworld-nodejs** repository. Update `kubernetes` `label` to **nodejs-app-pod-2** and the `image` for the **nodejs** `container` to be `node:10.10.0-alpine`.
